...
object App {
  ...
  def doesBoardHaveBingo(b: Board): Boolean = {
    ...
    def checkDiagonals: Boolean = {
      ...
      val topLeftToBottomRight =
        positions.map(p => b.v(p)(p)).filter(_.marked).size == 5
      val topRightToBottomLeft =
        positions.map(p => b.v(4 - p)(p)).filter(_.marked).size == 5
  ...
  def findLastWinningScore(state: State): Int = {
    val pick = state.picksLeft.head
    val updatedBoards = state.boards.map(markSpace(_, pick))

    val remaining = updatedBoards.filter(b => !doesBoardHaveBingo(b))
    if (remaining.isEmpty)
        boardPoints(
          updatedBoards.filter(doesBoardHaveBingo).last,
          pick
        )
    else
        findLastWinningScore(
          State(
            state.picksLeft.tail,
            remaining
          )
        )
  }
  ...
  def replaceWith[V](
      list: List[V],
      shouldReplace: V => Boolean,
      newVal: V
  ): List[V] =
    list match {
      case h :: t if shouldReplace(h) =>
        newVal :: replaceWith(t, shouldReplace, newVal)
  ...
  def markSpace(board: Board, num: Int): Board = {
    Board(
      board.v.map(l =>
        replaceWith(l, (s: Space) => s.value == num, Space(num, true))
      )
    )
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result = findLastWinningScore(State(picks, boards))
